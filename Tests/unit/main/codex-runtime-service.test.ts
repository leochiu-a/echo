...
describe('CodexRuntimeService', () => {
  ...
  it('emits failed summary from stderr snapshot for unexpected errors', async () => {
    const service = new CodexRuntimeService()
    vi.spyOn(service as never, 'executeTurnWithTimeout').mockRejectedValue(new Error('raw failure'))
    ...
    expect(events).toEqual([{ type: 'started' }, { type: 'failed', message: 'CLI execution timed out.' }])
    expect(service.isRunning()).toBe(false)
  })
  ...
  it('swallows prewarm errors and forwards cancel/dispose to resetSession', async () => {
    ...
    expect(resetSpy).toHaveBeenNthCalledWith(1, expect.objectContaining({ message: 'Cancelled by user.' }))
    expect(resetSpy).toHaveBeenNthCalledWith(2, expect.objectContaining({ message: 'Runtime disposed.' }))
  })
})
