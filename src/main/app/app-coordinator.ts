...
import {
  GlobalHotkeyService,
  ensureShortcutLifecycle,
} from "@main/services/hotkey/global-hotkey-service";
...
export class AppCoordinator {
  ...
  async startRuntimeRun(request: RuntimeRunRequest, targetWindow: BrowserWindow): Promise<void> {
    ...
    const resolvedCommand = resolveSlashCommandPrompt(
      rawCommand,
      this.settingsService.availableSlashCommands(),
    );
      ...
      (event) => {
        if (!targetWindow.isDestroyed()) {
          targetWindow.webContents.send(
            ipcChannels.runtimeEvent,
            event satisfies RuntimeStreamEvent,
          );
        }
