...
export class CodexRuntimeService {
  ...
  private async executeTurn(
    ...
    return await new Promise<AppServerTurnOutcome>((resolve, reject) => {
      ...
      void this.sendRequest(
        "turn/start",
        turnStartParams(this.threadId as string, prompt, model, reasoningEffort),
      )
        .then((response) => {
  ...
  private handleIncomingLine(rawLine: string): void {
    ...
    if (method === "error" && this.currentTurn) {
      this.currentTurn.errorMessage =
        typeof params.message === "string" ? params.message : this.currentTurn.errorMessage;
    }
  }
